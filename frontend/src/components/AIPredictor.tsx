...
import styles from './AIPredictor.module.css';
...
export default function AIPredictor() {
  ...
  const runPrediction = async () => {
    ...
    try {
      ...
      // 获取历史数据
      const response = await lotteryApi.getHistory(1, 100);
      ...
      setStatus(`运算出错: ${error instanceof Error ? error.message : '请重试'}`);
    } finally {
      setLoading(false);
    }
  };
      ...
      {result && (
        <div className={styles.aiResult}>
          <div className={styles.predictionRow}>
            <span className={styles.label}>AI 预测:</span>
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              {result.front.map((num, idx) => (
                <span key={idx} className="ball red small" style={{ animation: `fadeIn 0.3s ease ${idx * 0.1}s both` }}>
                  {num.toString().padStart(2, '0')}
                </span>
              ))}
              <span className="ball blue small" style={{ animation: 'fadeIn 0.3s ease 0.6s both' }}>
